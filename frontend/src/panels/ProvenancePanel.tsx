  ...
  return (
    <div className="flex flex-col gap-5" data-panel="provenance">
      <section className="flex flex-col gap-2">
        <h3 className="text-sm font-semibold">{t('provenancePanel.figures')}</h3>
          ...
          {p.figures.map((f) => (
            <li key={f.file}>
              <button
                ...
                data-figure={f.file}
                className="flex w-full flex-col gap-1 rule bg-surface p-1 text-start hover:border-accent"
              >
                ...
                {f.thumb ? (
                  <span className="flex aspect-4/3 w-full items-center justify-center overflow-hidden bg-surface-2">
                    <img
                  ...
                  <span className="flex aspect-4/3 w-full items-center justify-center bg-surface-2 text-2xs text-ink-3">
                    {t('value.missing')}
                  </span>
                )}
                <span className="line-clamp-2 px-1 text-2xs text-ink-3">{f.file}</span>
              </button>
            </li>
          ))}
        </ul>
      ...
      <Dialog.Root open={Boolean(open)} onOpenChange={(o) => !o && setOpen(null)}>
        <Dialog.Portal>
          ...
          <Dialog.Content
            className="fixed inset-4 z-60 flex flex-col gap-2 overflow-auto rule bg-surface p-4 shadow-float lg:inset-12"
            aria-describedby={undefined}
          >
            <Dialog.Title className="text-sm font-semibold">{shown?.file}</Dialog.Title>
            {shown?.thumb ? (
              <img
            ...
            <Dialog.Close className="self-start rule px-2 py-1 text-xs">
              {t('common.close')}
            </Dialog.Close>
          </Dialog.Content>
        </Dialog.Portal>
