...
| **Backend PASS** | Phase 6 recorded a verdict on the backend half; the screen is still owed |
| **FAIL** | Phase 6 found a real defect. Fix it in this phase — Phase 6's no-fix rule was Phase 6's |

Nothing here says "in progress". The owner changes a row only after checking the
running app.

> **Updated 8 Aug 2026, after merging `origin/main`.** Abd's, Karam's and Nizar's
> Phase 6 passes landed while this plan was being written and moved twelve rows.
> Phase 6 now carries **26 verdicts, not 14**, and a **second FAIL** (`p4-F`).
...
| core-C | Plume / particle engine | `/dashboard` plume layer, `/dashboard/replay` | Abd | **Backend PASS** (Phase 6, Abd) · UI Shell | Probability field rendered (never a trajectory line), `plume_source` shown verbatim, zero-current caveat on screen |
...
| ID | Feature | Route(s) | Owner | State today | The one thing that makes it Done |
|---|---|---|---|---|---|
| p4-01 | Storm Replay Mode | `/dashboard/replay/:eventId` | Abd | **Backend PASS** · frontend half open | Rainfall → runoff → sediment → plume → exposure plays in sequence from real frames |
| p4-02 | Live Forecast Mode | `/dashboard` (Forecast) | Nizar | **Backend PASS** with a staleness finding · frontend half open | A calm day reads as low risk, from `/forecast/latest` — see the architecture gap in `03-nizar.md` |
...
| p4-05 | Confidence Meter | `/dashboard` | Karam | **PASS — both halves** (the only fully-closed row) | Phrased exactly as "X% of ensemble members exceed this catchment's Nth-percentile rainfall" |
...
| p4-08 | Rain Intensity Ranking | `/events` | Karam | **Backend PASS** · frontend half open | Ranked on `rank`/`max_daily_mm` — **never** `max_anomaly_ratio` (stale column) |
...
| p4-14 | 3D Journey | journey overlay | Abd | **Backend PASS** · terrain/buildings/camera piece open | Six phases play; terrain assets present or the absence stated |
...
| p4-16 | Rainfall Accumulation Chart | `/dashboard` | Karam | **Backend PASS** · frontend half open | Small multiples, one shared scale, real rolling columns |
...
| p4-F | Multi-Source Weather Agreement | `/dashboard` | Nizar | 🔴 **FAIL (Phase 6)** — bare HTTP 500 | HYCOM vs Copernicus agreement, `null` rendered as "cache aged out", not 0 |
| p4-G | Historical Event Search | `/events` | Karam | **Backend PASS** · frontend half open | 675 events, filterable, each linking to its replay |
...
| p4-K | Seasonal Risk Calendar | `/events` | Karam | **Backend PASS** (data traced) · no surface yet | Month-by-month, framed as **rainfall intensity**, not exposure |
...
| b2 | Learned Transmission-Loss Model | — | Mahdi | **Absent — data now exists** | Karam found Cataldo et al. (2010), 90 measured values. Still not built; live value is the borrowed Negev proxy |
| b3 | Cross-Site Transfer Learning | — | Mahdi | **Absent — scoped out on purpose** | One site is the honest state; must not carry a badge implying transfer was tested |
...
| b6 | Live Anomaly Detection | `/dashboard` banner | Nizar | **PASS** — verified live, real anomalous + normal values | `anomaly_caveat` rendered **verbatim**; never conflated with the Confidence Meter |
...
Reconciled against `phase6/00-master-test-matrix.md` after the 8 Aug merge.

| State | Count |
|---|---|
| Done (frontend) | 3 — `core-D` (list), `p4-19`, `p4-A` |
| PASS both halves | 1 — `p4-05`, the only fully-closed row in the project |
| Backend PASS, screen owed | 10 — `core-C`, `p4-01`, `p4-02`, `p4-08`, `p4-14`, `p4-16`, `p4-G`, `p4-K`, `b6`, `p4-10` |
| Shell | 9 |
| Backend-only | 6 |
| Not started | 10 |
| Absent (no backend) | 4 — `b1`, `b2`, `b3`, `b9` |
| **FAIL, to be fixed this phase** | **2 — `p4-04`, `p4-F`** |
